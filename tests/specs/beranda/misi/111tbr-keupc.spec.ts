...
const dataTest = require('../../../datatest/ayotoko.data.ts');
    ...
    allureReporter.addTestId('[TOKO] 111. Submit Misi TBR (ke UPC)');
    allureReporter.addFeature('111. Submit Misi TBR (ke UPC)');
...
When(/^I view detail mission and submit form$/, async () => {
    ...
    await FormMisiScreen.formTBRJawabannUPC({upc2:dataTest.mission.inpupc2})
    await FormMisiScreen.lanjutkan()
    await FormMisiScreen.close()
});
