...
const dataTest = require('../../../datatest/ayotoko.data.ts');
    ...
    allureReporter.addTestId('[TOKO] 80. Orders Pick Up with promo shipping are rejected by WS, and vouchers are checked to see if they are available or not');
    allureReporter.addFeature('80. Orders Pick Up with promo shipping are rejected by WS, and vouchers are checked to see if they are available or not');

//Scenario Outline: As a user, I can order B2B with promo shipping for pick up
...
Then(/^System should be leading to Detail Transaksi screen and show order summary$/, async () => {
    ...
    await driver.pause(20000) //reject di Mitra
});
...
Then(/^System should be leading to Order detail screen$/, async () => {
    await OrderOnlineScreen.getDetailOrder1()
});
