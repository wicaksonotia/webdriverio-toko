...
const dataTest = require('../../../datatest/ayotoko.data.ts');
    ...
    allureReporter.addTestId('[TOKO] 52. Order Shipping One Day With Promo Shipping Get Poin');
    allureReporter.addFeature('52. Order Shipping One Day With Promo Shipping Get Poin');

//Scenario Outline: As a user, I can order B2B with promo shipping for sent today and get poin
...
Then(/^System should be leading to Detail Transaksi screen and show order summary$/, async () => {
    ...
    await OrderDetailsScreen.getTotalPoin()
