...
const dataTest = require('../../../datatest/ayotoko.data.ts');
    ...
    allureReporter.addTestId('[TOKO] 76. Order Shipping Tomorrow With Promo Percent Confirm Adjust Price');
    allureReporter.addFeature('76. Order Shipping Tomorrow With Promo Percent Confirm Adjust Price');

//Scenario Outline: As a user, I can order B2B with promo percent for sent tomorrow then confirm price changes
...
Then(/^System should be leading to Detail Transaksi screen and show order summary$/, async () => {
    ...
    await driver.pause(20000) //adjust price di Mitra
});
...
When(/^I click my order history and click button 'SETUJU'$/, async () => {
    ...
    await driver.pause(2000)
    await OrderDetailsScreen.confirmChanges()
    await driver.pause(5000)
});

Then(/^System should be able to update status to 'Perubahan Disetujui'$/, async () => {
    await OrderOnlineScreen.getDetailOrder1()
    await OrderOnlineScreen.view1HistoryProdukMitra()
    await OrderDetailsScreen.waitForIsShown(true)
    await OrderDetailsScreen.page()
